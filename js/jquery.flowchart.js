    ...
    $.widget("flowchart.flowchart", {
        ...
        _initEvents: function () {
            ...
            this.objs.layers.operators.on('click', '.flowchart-operator-connector-arrow', function () {
                var $this = $(this).closest( '.flowchart-operator-connector' );
            ...
            this.objs.layers.operators.on('input propertychange', '.flowchart-operator-connector-label', function () {
                var $this = $(this);
								var oid = $this.closest( '.flowchart-operator' ).data( 'operator_id' );
								var ctype = $this.closest('.flowchart-operator-connector-set').data('connector_type');
								var cid = $this.closest( '.flowchart-operator-connector' ).data( 'connector' )
								self.data.operators[ oid ].properties[ ctype ][ cid ].label = $this.val();
            });

            this.objs.layers.operators.on('input propertychange', '.tags', function () {
                var $this = $(this);
								var oid = $this.closest( '.flowchart-operator' ).data( 'operator_id' );
								self.data.operators[ oid ].properties[ "tags" ] = $this.val();
            });

            this.objs.layers.operators.on('input propertychange', '.stats', function () {
                var $this = $(this);
								var oid = $this.closest( '.flowchart-operator' ).data( 'operator_id' );
								self.data.operators[ oid ].properties[ "stats" ] = $this.val();
            });
        ...
        _getOperatorFullElement: function (operatorData) {
            ...
            $operator_inputs_outputs.appendTo($operator);
						var $operator_row = $( '<div style="display:table-row"></div>' )
						$operator_row.appendTo( $operator_inputs_outputs );

            var $operator_inputs = $('<div class="flowchart-operator-inputs"></div>');
            $operator_inputs.appendTo($operator_row);

            var $operator_outputs = $('<div class="flowchart-operator-outputs"></div>');
            $operator_outputs.appendTo($operator_row);

            var body= $('<div class="fldata"></div>');
						body.appendTo($operator_inputs_outputs);
						$( '<div>tags</div>' ).appendTo( body );
						var bodyTextArea = $( '<textarea class="tags"></textarea>' );
						bodyTextArea.val( infos.tags || '' )
						bodyTextArea.appendTo( body );

            body= $('<div class="fldata"></div>');
						body.appendTo($operator_inputs_outputs);
 						$( '<div>stats</div>' ).appendTo( body );
						var bodyTextArea = $( '<textarea class="stats"></textarea>' );
						bodyTextArea.val( infos.stats || '' )
						bodyTextArea.appendTo( body );
        ...
        _createSubConnector: function (connectorKey, connectorInfos, fullElement) {
            ...
            var $operator_connector_label = $('<textarea class="flowchart-operator-connector-label"></textarea>');
